...
//       lessMessage: studentMessage || "defaultMass"
//     };
//     console.log(data);
...
const CalendarLecc = ({ scheduleLec, isLec, connectionLecId, specificConnection }) => {
  ...
  const [currentLessonL, setCurrentLessonL] = useState(null);
  const [messageL, setMessageL] = useState(""); // הוספת מצב להודעה
  const [lessonData, setLessonData] = useState({ lesson: { lessMessage: [] } });
  const [studentMessage, setStudentMessage] = useState('');
  console.log(lessonData);
  ...
  const handleSendingMessage = async (e) => {
    ...
    const data = {
      lessID: lessonData?.lesson?.lessID || "defaultID", // החלף "defaultID" בערך ברירת המחדל שאתה רוצה
      lessMessage: studentMessage
    };
    console.log(data);

    try {
      const res = await axios.put(`http://localhost:3008/api/lessonsStudLec/messages/${data.lessID}`, { lessMessage: studentMessage, withCredentials: true });
      console.log('Message sent successfully');
      alert('הודעה נשלחה');
      handleClosePopupBocd(); // סגירת הפופאפ לאחר שליחת ההודעה
    } catch (error) {
